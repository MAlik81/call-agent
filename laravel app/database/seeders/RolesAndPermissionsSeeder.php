...
class RolesAndPermissionsSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Fetch or create the global admin role
        ...
        if ($role->is_tenant_role !== false) {
            $role->is_tenant_role = false;
            $role->save();
        }

        // Give all non-tenant permissions to the global admin role
        $role->givePermissionTo(
            Permission::all()->filter(function ($permission) {
                return !str_starts_with($permission->name, TenancyPermissionConstants::TENANCY_PERMISSION_PREFIX);
            })
        );
    ...
    private function multiTenancyRolesAndPermissions()
    {
        ...
        // Ensure is_tenant_role is true
        if (!$adminRole->is_tenant_role) {
            $adminRole->is_tenant_role = true;
            $adminRole->save();
        }
        ...
        if (!$userRole->is_tenant_role) {
            $userRole->is_tenant_role = true;
            $userRole->save();
        }

        // You can assign specific permissions to $userRole here if needed
    }
}
