...
class CreatePermissionTables extends Migration
{
    public function up(): void
    {
        ...
        if ($teams && empty($columnNames['team_foreign_key'] ?? null)) {
            throw new \Exception('Error: team_foreign_key missing in config/permission.php.');
        }
            ...
            Schema::table($tableNames['roles'], function (Blueprint $table) use ($teams, $columnNames) {
                if ($teams || config('permission.testing')) {
                    if (DB::getDriverName() === 'mysql') {
                        DB::statement(
                            "ALTER TABLE `roles` ADD UNIQUE `roles_team_name_guard_name_unique` (`{$columnNames['team_foreign_key']}`, `name`(100), `guard_name`(100))"
                        );
                    } else {
                    ...
                    if (DB::getDriverName() === 'mysql') {
                        DB::statement(
                            "ALTER TABLE `roles` ADD UNIQUE `roles_name_guard_name_unique` (`name`(100), `guard_name`(100))"
                        );
                    } else {
        ...
        if (Schema::hasTable($tableNames['permissions'])) {
            Schema::table($tableNames['permissions'], function (Blueprint $table) {
                if (DB::getDriverName() === 'mysql') {
                    DB::statement(
                        "ALTER TABLE `permissions` ADD UNIQUE `permissions_name_guard_name_unique` (`name`(100), `guard_name`(100))"
                    );
                } else {
