<div {{ $attributes->merge(['class' => 'flex flex-col rounded-2xl shadow-xl overflow-hidden transition hover:scale-103']) }}>
    ...
    <div class="flex flex-col flex-wrap gap-1 mb-6 flex-grow align-items-stretch p-4 mt-1">
        ...
        @if ($post->blogPostCategory)
            <div
                class="text-neutral-400 rounded-lg px-2 py-1 border border-neutral-300 max-w-fit text-xs mt-3 hover:bg-neutral-100">
                <a
                    href="{{route('blog.category', ['slug' => $post->blogPostCategory->slug])}}">{{ $post->blogPostCategory->name }}</a>
            </div>
        @endif
    </div>
